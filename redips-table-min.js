...
May 7, 2012.
*/
"use strict";var REDIPS=REDIPS||{};REDIPS.table=(function(){var onmousedown,handler_onmousedown,merge,merge_cells,max_cols,split,get_table,mark,cell_init,row,column,cell_list,relocate,remove_selection,cell_index,tables=[],td_event,show_index,color={cell:false,row:false,column:false};onmousedown=function(el,flag,type){var td,i,t,get_tables;get_tables=function(el){var arr=[],nodes,i;nodes=el.getElementsByTagName('table');for(i=0;i<nodes.length;i++){arr.push(nodes[i]);}
...
cell_index();};cell_init=function(c){if(td_event===true){REDIPS.event.add(c,'mousedown',handler_onmousedown);}
else{REDIPS.event.remove(c,'mousedown',handler_onmousedown);}};handler_onmousedown=function(e){var evt=e||window.event,td=evt.target||evt.srcElement,mouseButton;if(evt.which){mouseButton=evt.which;}
...
return tbl;};row=function(table,mode,index){var nc,nr,fr,c,cl,cols=0,i,j,k;remove_selection();if(typeof(table)!=='object'){table=document.getElementById(table);}
...
i+=c.colSpan-1;}}};column=function(table,mode,index){var c,idx,nc,i;remove_selection();if(typeof(table)!=='object'){table=document.getElementById(table);}
if(index===undefined){index=-1;}
...
catch(error){}}};cell_list=function(table){var matrix=[],matrixrow,lookup={},c,ri,rowspan,colspan,firstAvailCol,tr,i,j,k,l;tr=table.getElementsByTagName('TR');for(i=0;i<tr.length;i++){for(j=0;j<tr[i].cells.length;j++){c=tr[i].cells[j];ri=c.parentNode.rowIndex;rowspan=c.rowSpan||1;colspan=c.colSpan||1;matrix[ri]=matrix[ri]||[];for(k=0;k<matrix[ri].length+1;k++){if(typeof(matrix[ri][k])==='undefined'){firstAvailCol=k;break;}}
